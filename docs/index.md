...
public TableNameResolver tableNameResolver() {
    return (domainClass, baseTableName) -> {
        ...
        String env = environment.getProperty("app.environment", "dev");
        return env + "_" + baseTableName;
    };
}
```
...
public class Product {
    ...
    private String productId;  // Automatically generated UUID
...
public class Product {
    ...
    private String productId;  // Auto-generated UUID (SDK_V1_COMPATIBLE only)
...
public class AuditEntity {
    ...
    private Date createdAt;  // Set only on create

    @DynamoDBAutoGeneratedTimestamp(strategy = DynamoDBAutoGenerateStrategy.ALWAYS)
    private Date updatedAt;  // Set on create and update
}
```
...
public TableNameResolver tableNameResolver() {
    return (domainClass, baseTableName) -> {
        // Map all entities to a single table
...
public class Order {
    ...
    @DynamoDbPartitionKey
    private String customerId;  // Main table hash key

    @DynamoDbSortKey
    private String orderDate;  // Main table range key
...
public class Product {
    ...
    @DynamoDbPartitionKey
    private String productId;  // Access by ID

    @DynamoDbSecondaryPartitionKey(indexNames = {"category-index"})
    private String category;  // Query by category

    @DynamoDbSecondaryPartitionKey(indexNames = {"seller-index"})
    private String sellerId;  // Query by seller
}
```
...
@DynamoDbPartitionKey
private String customerId;  // Partition by customer

@DynamoDbSortKey
private String orderDate;  // Sort by date
...
// Use GSI instead of scan to reduce read costs
...
public TableNameResolver tableNameResolver() {
    return (domainClass, baseTableName) -> {
        ...
        return env != null ? env + "_" + baseTableName : baseTableName;
    };
}
```
